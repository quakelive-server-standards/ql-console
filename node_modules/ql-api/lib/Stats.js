...
var ql_stats_model_1 = require("ql-stats-model");
...
var Stats = /** @class */ (function (_super) {
    ...
    function Stats(address, password, options) {
        ...
        _this.onMessage(function (message) {
            ...
            var type = obj.TYPE;
            var data = obj.DATA;
            _this.events.emit('RAW_QL_EVENT', json);
            switch (type) {
                case 'MATCH_REPORT':
                    _this.events.emit('MATCH_REPORT', ql_stats_model_1.MatchReportEvent.fromQl(data));
                    break;
                case 'MATCH_STARTED':
                    _this.events.emit('MATCH_STARTED', ql_stats_model_1.MatchStartedEvent.fromQl(data));
                    break;
                case 'PLAYER_CONNECT':
                    _this.events.emit('PLAYER_CONNECT', ql_stats_model_1.PlayerConnectEvent.fromQl(data));
                    break;
                case 'PLAYER_DEATH':
                    _this.events.emit('PLAYER_DEATH', ql_stats_model_1.PlayerDeathEvent.fromQl(data));
                    break;
                case 'PLAYER_DISCONNECT':
                    _this.events.emit('PLAYER_DISCONNECT', ql_stats_model_1.PlayerDisconnectEvent.fromQl(data));
                    break;
                case 'PLAYER_KILL':
                    _this.events.emit('PLAYER_KILL', ql_stats_model_1.PlayerKillEvent.fromQl(data));
                    break;
                case 'PLAYER_MEDAL':
                    _this.events.emit('PLAYER_MEDAL', ql_stats_model_1.PlayerMedalEvent.fromQl(data));
                    break;
                case 'PLAYER_STATS':
                    _this.events.emit('PLAYER_STATS', ql_stats_model_1.PlayerStatsEvent.fromQl(data));
                    break;
                case 'PLAYER_SWITCHTEAM':
                    _this.events.emit('PLAYER_SWITCHTEAM', ql_stats_model_1.PlayerSwitchTeamEvent.fromQl(data));
                    break;
                case 'ROUND_OVER':
                    _this.events.emit('ROUND_OVER', ql_stats_model_1.RoundOverEvent.fromQl(data));
                    break;
                default:
    ...
    Stats.prototype.onRawQlEvent = function (listener) {
